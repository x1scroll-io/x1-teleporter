...
// bridge — sol_x1, x1, x1_reverse, x1_onward) the LiFi integrator fee MUST be
// 0: the stage-2 on-chain skim is the only Teleporter fee on that class, and
// Warp's $1 flat is a third-party pass-through. Non-X1 (same-chain) routes
// keep the 1% integrator — that IS their once-per-journey Teleporter fee.
//
...
// ALWAYS overwritten server-side, so the browser can neither strip the 1% on
...
/** Decide the integrator fee the server will FORCE for this quote request.
 *  Pure + exported so the fee policy is unit-testable without a live LiFi
 *  call. Returns "0" for x1-class requests, INTEGRATOR_FEE otherwise.
 *
...
export function resolveForcedFee(params) {
  ...
  return touchesSolana ? "0" : INTEGRATOR_FEE;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    params.set("fee", resolveForcedFee(params));
