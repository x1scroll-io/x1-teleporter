...
```
3. You: merge it. Watch x1teleporter.com — it should be identical to before.
4. You: flip `VITE_FLAG_THORCHAIN=true` in Production. Announce to Jack. Leave `ANYSWAP` off until the ten-route test passes in week three.
5. If anything looks wrong: flag off → Vercel Instant Rollback → `git revert`, in that order.
