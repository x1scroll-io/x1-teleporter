...
## Step 2.2 implementation status (additive note — canonical content above untouched)

- Discovery: `src/lib/wallet/evmDiscovery.js` (wagmi EIP-6963 — connectors from
  `eip6963:announceProvider`, per the EVM section above) and
  `src/lib/wallet/solanaDiscovery.js` (Wallet Standard registry →
  StandardWalletAdapter), composed in `src/lib/wallet/walletDiscovery.js`
  (`getDiscovered` / `getProvider(family, walletId)` / `subscribe`).
- Connect modal: `src/components/ConnectModal.jsx` inside the ONE-card tab
  layout (`BridgeCard.jsx` — Teleport / THORChain / Buy tabs). Rules above
  implemented in `src/lib/wallet/modalLogic.js` (pure, tested without a
  browser): fixed order per family (Starport pinned → reference wallet →
  alphabetical rest), installed highlighted in place, not-installed shown
  with install links, never hidden; announced providers outside the registry
  get appended entries (docs/BRIEF.md wallet layer).
- Small registry map only (a few rows per family, Starport + reference +
  a couple of majors). The full canonical tables above get wired in later
  steps behind the same modalLogic.
- Phantom multi-chain: separate entries in EVM (rdns `app.phantom`), Solana,
  and Bitcoin lists — separate sessions, never merged (multi-family rule
  above).
- Grep rule: `src/lib/wallet/noWindowProbe.test.js` fails the build if any
  injected global (EVM / Solana / Bitcoin / Tron) is read in src/ (legacy
  allowlist: Teleporter.jsx, warpBridge.js — Phase 3 removal).
- Mock provider (Step 2.1) remains the fallback (Starport + tests); live
  provider connect is the operator's browser preview check. NOT wired into
  main.jsx yet — Teleporter.jsx stays live until the Phase 3 swap.
