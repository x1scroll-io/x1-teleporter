...
**Scaffold consequence:** `TOKENS.rbn = { USDG }` only (the resolver's
TOKEN_TABLE USDG row — see §7). No USDC entry was invented. The X1 hop still
lands **Solana USDC** (LiFi/Relay converts USDG→USDC inside the route), so the
Warp leg and X1 USDC.x minting are unchanged.
...
## 6. Token resolver relevance (resolved by PR #57)

RH Chain USDC is a **non-event for the resolver**: there is no canonical
Circle USDC contract on Robinhood Chain to map — the USDC row keeps its rbn
entry unverified (`resolve("USDC","rbn")` → null; `syrupUSDC` is NOT mapped
as USDC). The canonical stable landed as a dedicated **USDG row** at the END
of `TOKEN_TABLE` (canonical contract
`0x5fc5360D0400a0Fd4f2af552ADD042D716F1d168`, 6 dec — the same address LiFi
lists), so `resolve("USDG","rbn")` resolves and the TOKENS projection gains
`rbn.USDG`.

## 7. What was scaffolded (branch feat/robinhood-chain → PR #57, merged into v2)

1. `src/lib/tokenResolver.js` — canonical **USDG row** appended at the END of
   `TOKEN_TABLE` (rbn entry: address `0x5fc5360D0400a0Fd4f2af552ADD042D716F1d168`,
   6 dec, erc20, lifi rail, listed). The USDC row's rbn entry stays
   unverified (null address → `resolve("USDC","rbn")` stays null).
2. `src/lib/teleportConstants.js` — `CHAINS.rbn` (lifiKey `out`, chainId
   4663, walletType `evm`, green `#00C805`, glyph `R`); `TOKENS.rbn` comes
   from the resolver projection (`{ USDG }`); `EVM_CHAINS` appends `rbn` →
   console pickers (`SOURCE_CHAINS`, `tokensOn`) surface it automatically.
3. `src/Teleporter.jsx` — v1 safety-net mirror kept in sync (same `CHAINS.rbn`
   + `TOKENS.rbn` entries, id `rbn` to match the resolver; v1's ChainSelect
   enumerates CHAINS dynamically).
4. `src/lib/prices.js` — `COINGECKO_IDS.USDG = "global-dollar"` (CG id
   verified live at $1.00) so the balance line shows USDG in USD.
5. `test/fixtures/golden/robinhood-leg/` — SIMULATED quote evidence (README +
   2 li.quest captures + re-run commands), additive-only, frozen files
   untouched.
6. `src/lib/teleportConstants.test.js` — 6 tests: chain entry validity,
   the no-fake-USDC honesty test, rail routing, quote-builder params,
   fixture structure. Registered in package.json test list.
7. `src/lib/prices.test.js` — updated mocks/assertions for the new
   COINGECKO_IDS key (additive; no frozen file touched).

## 8. Ready for live test (Mr. Esters)

- Console forward journey: source **Robinhood Chain (rbn) → USDG → X1 (USDC.x)** —
  verify the quote renders (Relay route), then a small ($25–50) live bridge.
- Console reverse: **X1 (USDC.x) → Robinhood Chain (rbn) → USDG**.
