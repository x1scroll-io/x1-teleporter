 ...
 *     in the tx). The LiFi query is SOL → the destination EVM chain, marked
 *     x1Class=1 so the server omits the LiFi integrator fee entirely (policy:
 *     the 1% warp-skim is the ONLY Teleporter fee on x1-class routes;
 *     api/lifi/quote.js validates the marker against Solana on one end of the
 *     leg). The Solana-side FROM token is token-aware too:
 ...
 *                 decimals (6 | 9).
 *   toToken     = the destination stable on the EVM chain (default "USDC";
 *                 the #36 dest-choice selector passes USDT/DAI here).
 ...
 */
export function buildReverseLifiQuoteParams({ to, netOnSolana, fromAddress, toAddress, slippage = 0.5, token = "USDC.x", toToken = "USDC" }) {
  const toChain = CHAINS[to]?.lifiKey;
  const toTokenAddr = TOKENS[to]?.[toToken]?.address;
  if (!toChain || !toTokenAddr) return null;
  ...
  const qs = new URLSearchParams({
    ...
    toToken: toTokenAddr,                 // lands as the chosen stable on the destination EVM chain
  ...
  return { qs, decimals: fromDecimals, feeUsed: null };
}
...
export function deriveReverseQuote({ data, to, amount, token = "USDC.x", toToken = "USDC" }) {
  ...
  let out;
  if (lifiQuoted) {
    // LiFi delivers the stable on the destination EVM chain — its decimals.
  ...
  return {
    ...
    net: out,
    recvToken: lifiQuoted ? toToken : solanaSymbol,
