/**
 * Dogecoin wallet discovery (Step 2.4) — the registry's Dogecoin table,
 * implemented with namespaced-global detection + minimal per-table connect
 * adapters.
 *
 * Detection (docs/WALLET-REGISTRY.md, Dogecoin table): namespaced globals
 * only — window.xfi.dogecoin (Ctrl), window.doge (MyDoge, key marked
 * "(verify)"), window.dogeLabs (DogeLabs, key marked "(verify)"). No
 * Wallet Standard registration for DOGE and no bare legacy global to
 * impersonate, so no impersonation rule is needed here.
 *
 ...
 * row is a ⚠️ row (MyDoge, DogeLabs, Enkrypt, OKX, Trust, Bitget), a
 * hardware row (Ledger/Trezor — Phase 3 XChainJS lane), or the
 * deposit-address row — ALL of them reject connect() with their
 * clearly-marked TODO. No guessed APIs, no fake "mock" connects.
 *
 * MEMO RULE (memoRule.js): Ctrl carries memoSupport "op_return"; every
 * other row carries "verify" — the modal shows the hand-off note and sends
 * route to the deposit-address row until verified (the registry's MyDoge
 * note: "if absent, it's balance-only + deposit-address").
 *
...
const GLOBAL_DETECTORS = Object.freeze({
  [DOGECOIN_WALLET_IDS.CTRL]: (w) => Boolean(w?.xfi?.dogecoin),
  [DOGECOIN_WALLET_IDS.MYDOGE]: (w) => Boolean(w?.doge),
  [DOGECOIN_WALLET_IDS.DOGELABS]: (w) => Boolean(w?.dogeLabs),
});
...
export function enumerateGlobalWallets(win = defaultWin()) {
  if (!win) return [];
  return DOGECOIN_WALLETS.filter(
...
/**
 * Create the Dogecoin discovery handle — same shape as the other family
 ...
 *     createDogeBalanceFetcher in the app). Defaults to null (connect then
 *     returns no balance).
 */
export function createDogecoinDiscovery({
  win = undefined,
  balanceFetcher = null,
  onChange = () => {},
} = {}) {
  ...
  return {
    /** Take the initial snapshot (no late-announce subscriptions — DOGE has no registry). */
    start() {
      refresh();
    },
    ...
    getProvider(walletId) {
      ...
      return createDogecoinProviderAdapter({
        walletId,
        win: win ?? defaultWin(),
        balanceFetcher,
      });
...
export function firstDogeAddress(accounts) {
...
function createUnverifiedProvider(entry) {
  ...
  return {
    family: "dogecoin",
    id: `dogecoin:${entry.id}`,
    isReal: true,
    walletName: entry.name,
    unverified: true,

    async connect() {
      throw new Error(`Dogecoin wallet "${entry.name}" is not wired yet — ${note}`);
    },
...
/**
 * Ctrl (ex-XDEFI) — memo supported per the registry. Connect API:
 * request({ method: "request_accounts" }) on window.xfi.dogecoin (the same
 * xfi-family surface the Bitcoin table documents). Returns the user's
 * selected DOGE addresses; the first is the session address. Memo-on-send
 * is a later THORChain-lane step — NO signing here.
 */
function createCtrlAdapter(entry, { win, balanceFetcher }) {
  return {
    ...
    async connect() {
      const api = win?.xfi?.dogecoin;
      if (!api || typeof api.request !== "function") {
        throw new Error("Ctrl (ex-XDEFI) is not available (window.xfi.dogecoin missing)");
      }
      const result = await api.request({ method: "request_accounts" });
      const accounts = Array.isArray(result) ? result : result?.addresses;
      const address = firstDogeAddress(accounts);
      ...
      return { family: "dogecoin", address, balance, provider: this };
    },
...
/**
 * Build the provider adapter for a Dogecoin registry wallet.
 ...
 */
export function createDogecoinProviderAdapter({
  walletId,
  win = defaultWin(),
  balanceFetcher = null,
  registry = DOGECOIN_WALLETS,
}) {
  const entry = registry.find((e) => e.id === walletId);
  if (!entry) return null;

  // ⚠️ rows (MyDoge/DogeLabs/Enkrypt/OKX/Trust/Bitget — unverified APIs)
  // never fake a connect — reject with the verification note.
  if (entry.status === "verify" || entry.connectTodo) return createUnverifiedProvider(entry);

  if (walletId === DOGECOIN_WALLET_IDS.CTRL) {
